...
for (const caseNumber of [1, 2, 3, 4, 5]) {
	describe(`Custom 404 with implicit rerouting - Case #${caseNumber}`, () => {
		...
		describe('dev server', () => {
			...
			// sanity check
			it('dev server handles normal requests', { timeout: 10000 }, async () => {
				const response = await fixture.fetch('/');
				assert.equal(response.status, 200);
			});

			// IMPORTANT: never skip
			it('dev server stays responsive', { timeout: 10000 }, async () => {
		...
		describe('prod server', () => {
			...
			// sanity check
			it('prod server handles normal requests', { timeout: 10000 }, async () => {
